...
**Date:** 2026-03-30
**Status:** Approved (rev 3 — adds post-implementation review amendments from architecture, security, regression, and usability agents)
...
## Rev 3 Amendments — Post-Implementation Review Findings (2026-03-30)

Findings from architecture, security, regression, and usability reviews conducted after Plan 1 (migration + code switch) shipped. These amendments apply to Plan 2 (admin UI + sync).

### A1. Stock is read-only in variations PUT (Critical — Architecture)

The `PUT /api/admin/inventory/[id]/variations` endpoint must **never accept `stock_count`** in the payload. Between fetch and save, a concurrent sale could decrement stock — the PUT would silently overwrite it with stale data. Stock is displayed read-only in the variations table. Stock adjustments happen only through:
- `decrement_variation_stock` / `increment_variation_stock` RPCs
- A separate "Adjust Stock" action per variation row (writes `stock_movements` with `manual_adjustment`)

### A2. Full-replace via Postgres RPC, not multi-round-trip JS (Critical — Architecture)

The PUT handler must not diff-and-delete across multiple JS round-trips. Implement as a single `replace_product_variations(product_id, options_json, variations_json)` RPC that runs inside `BEGIN...COMMIT`. This prevents race conditions where a concurrent checkout or webhook modifies variation rows mid-diff.

### A3. Default variation lifecycle — explicit rules (Critical — Architecture)

**Adding options to a simple product:**
- The existing default variation is **deactivated** (`is_active = false`, `is_default = false`), not deleted — preserving `stock_movements` FK history.
- A new default is selected from the first generated combination.
- Stock from the old default does NOT transfer — new variations start at stock = 0 (as designed).

**Removing all options (reverting to simple):**
- The handler detects `options: []` and consolidates: picks the variation with the highest stock, marks it `is_default = true`, deactivates all others.
- Sets `products.has_options = false` (trigger handles this via `product_options` DELETE).

### A4. Payload size limits (Critical — Security)

Reject at the top of the PUT handler before any DB work:
- `options.length > 3` → 400
- Any `option.values.length > 20` → 400
- `variations.length > 60` → 400

### A5. Price must be positive (Critical — Security)

Enforce `price > 0` (not `>= 0`) on all variation prices in the PUT handler. Also add a DB constraint in the migration:
```sql
ALTER TABLE product_variations ADD CONSTRAINT chk_pv_price_positive CHECK (price > 0);
```
This prevents free checkout via $0 price manipulation.

### A6. IDOR ownership check (Critical — Security)

Every update of an existing variation or option ID must verify ownership:
```typescript
.eq('id', incoming.id).eq('product_id', productId)
```
Without this, a crafted PUT payload could overwrite variations belonging to other products.

### A7. Soft-delete for sold variations (High — Architecture)

Never hard-delete variations that have `order_line_items` or `stock_movements` rows. The PUT handler must:
1. Check `order_line_items` for any variation being removed
2. If found → `is_active = false` (soft-delete)
3. Hard-delete only variations with zero orders and zero stock movements

Also: `saved_list_items.variation_id` is `ON DELETE SET NULL` — the storefront saved-list feature must handle null `variation_id` gracefully.

### A8. Chip-based value input, not comma-separated (Must — UX)

Replace comma-separated text input with individual "Add" button + Enter key. Each value appears as a removable chip immediately. This prevents parsing ambiguity for non-technical admins (e.g., values containing commas).

### A9. Live combination preview count (Should — UX)

When adding option values, show a live count: "Adding 'XL' will create 3 new variations." This teaches the combinatorial model incrementally and prevents surprise row generation.

### A10. SKU field sanitization (High — Security)

```typescript
sanitizeText(String(v.sku)).slice(0, 100)
```
Validate alphanumeric + hyphens/underscores only. Hide the SKU column by default in the variations table — most handmade shop products don't use SKUs.

### A11. Square pull must sanitize ITEM_OPTION names (High — Security)

When `pullProductsFromSquare` is extended to import `ITEM_OPTION` objects, wrap all name fields with `sanitizeText()` — identical to the existing treatment for product names.

### A12. Private sale checkout — use variation_id when present (High — Regression)

`app/api/shop/private-sale/[token]/checkout/route.ts` hardcodes `.eq('is_default', true)` for stock decrement. Once the admin UI can set `private_sale_items.variation_id` to a non-default variation, the checkout must use that value instead.

### A13. Hotfix shipped: channel_sync_log upsert (Critical — Already Fixed)

Migration 048 replaced the unique constraint with partial indexes, breaking PostgREST's `onConflict`. Fixed by replacing `upsert` with select+update/insert pattern in `lib/channels/index.ts` and `lib/channels/square/webhook.ts`. Deployed to production 2026-03-30.

### A14. Hotfix shipped: PATCH has_options guard (Critical — Already Fixed)

PATCH `/api/admin/inventory/[id]` now skips the default-variation price/stock sync when `data.has_options === true`. Deployed to production 2026-03-30.

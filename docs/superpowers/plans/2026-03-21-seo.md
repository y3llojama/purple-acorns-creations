...
**Architecture:** A new `lib/seo.ts` module exports pure typed schema builder functions plus a `JsonLd` server component. Pages import `JsonLd` and pass it a schema object — the component handles safe inline script rendering internally. This keeps the sensitive render pattern in one auditable location.
...
|---|---|---|
| `lib/seo.ts` | **Create** | Schema builder functions + `JsonLd` component |
...
## Task 1: Schema Builders + JsonLd Component (`lib/seo.ts`)
...
```bash
git add lib/seo.ts __tests__/lib/seo.test.ts
git commit -m "feat: add JSON-LD schema builders and JsonLd component (lib/seo.ts)"
```

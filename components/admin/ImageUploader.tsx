...
export default function ImageUploader({ bucket, onUpload, label = 'Upload Image' }: Props) {
  ...
  return (
    <div>
      ...
      <label
        htmlFor="file-input"
        style={{ display: 'inline-flex', alignItems: 'center', padding: '12px 24px', background: altText.trim() ? 'var(--color-primary)' : '#ccc', color: '#fff', borderRadius: '4px', cursor: altText.trim() ? 'pointer' : 'not-allowed', fontSize: '16px', minHeight: '48px' }}
      >
